...
<div class='field'><label for='simulation_mode'>Simulation mode</label><select id='simulation_mode'><option value='start-line'>Start Line</option><option value='finish-line'>Finish Line</option></select></div>
<div class='field'><label for='duration_hours'>Duration (hours)</label><input id='duration_hours' type='number' min='0.01' step='0.1'></div>
...
function form(){const c={};ids.forEach(id=>{const e=document.getElementById(id);c[id]=e.type==='checkbox'?e.checked:(e.type==='number'?Number(e.value):e.value)});const text=document.getElementById('noise_tags').value.trim();if(text){const parsed=JSON.parse(text);if(!Array.isArray(parsed)||!parsed.every(x=>typeof x==='string'))throw new Error('Custom noise tags must be a JSON array of strings.');c.noise_tags=parsed}else c.noise_tags=null;return c}
...
async function loadScenario(){try{const id=document.getElementById('scenario').value;const s=await request('/test/scenarios/'+encodeURIComponent(id));setForm(s.settings);updateDescription();showError('')}catch(e){showError(e.message)}}
async function load(){try{const s=await request('/test/status');if(!initialized){await loadScenarios(s.scenario_id);setForm(s.config);initialized=true}render(s);showError('')}catch(e){showError(e.message)}}
